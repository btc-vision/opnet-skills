...
This guide is for developers who want to contribute to the WalletConnect library or add support for new wallets.
...
│   ├── opwallet/               # OP_WALLET implementation
│   │   ├── controller.ts
│   │   └── interface.ts
...
- Handles wallet switching and cleanup
...
interface WalletBase {
  ...
  // MLDSA (quantum-resistant signatures)
...
Create `interface.ts` with the wallet's browser API types:

```typescript
// src/wallets/mywallet/interface.ts
import type { Unisat, UnisatChainInfo, UnisatChainType } from '@btc-vision/transaction';
...
class MyWallet implements WalletBase {
  ...
  async canAutoConnect(): Promise<boolean> {
    // Return true if wallet can reconnect without user interaction
  ...
  async getSigner(): Promise<UnisatSigner | null> {
    // Return UnisatSigner if your wallet supports it
...
If your wallet doesn't support MLDSA, return `null` or `false` from these methods.
...
4. Test connection, disconnection, account switching, and network changes
