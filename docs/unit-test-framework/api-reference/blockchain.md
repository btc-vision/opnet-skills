...
> **WARNING:** `medianTimestamp` exists for completeness but should NOT be used for time-dependent test logic (deadlines, staleness, durations). Use `blockNumber` instead. Bitcoin's Median Time Past can be manipulated by miners within a +/- 2 hour window. The actual btc-runtime contracts (OP20, OP721, OP20S, Upgradeable) ALL use `block.number` for deadlines.
...
| `medianTimestamp` | `bigint` | `BigInt(Date.now())` | Block median time -- **DO NOT USE for time-dependent test logic; use `blockNumber` instead** |
