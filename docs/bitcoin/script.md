...
```typescript
function decode(buffer: Uint8Array, maxLength?: number, minimal?: boolean): number;
```

| Parameter | Default | Description |
|-----------|---------|-------------|
| `buffer` | -- | The Uint8Array containing the encoded script number |
...
```typescript
function encode(number: number): Uint8Array;
```

Encodes a JavaScript number into a minimally-encoded script number Uint8Array.
...
// buf => Uint8Array [0xe8, 0x03]

const negBuf = script.number.encode(-1000);
// negBuf => Uint8Array [0xe8, 0x83]

// Decode
script.number.decode(new Uint8Array([0xe8, 0x03])); // 1000
script.number.decode(new Uint8Array([0xe8, 0x83])); // -1000
script.number.decode(new Uint8Array([]));            // 0

// 5-byte locktime value
script.number.decode(new Uint8Array([0x01, 0x00, 0x00, 0x00, 0x00]), 5); // 1
```
...
script.number.encode(1);     // Uint8Array [0x01]
script.number.encode(127);   // Uint8Array [0x7f]
script.number.encode(128);   // Uint8Array [0x80, 0x00]  (extra byte to avoid sign-bit collision)
script.number.encode(-1);    // Uint8Array [0x81]
script.number.encode(-128);  // Uint8Array [0x80, 0x80]

// Decode with strict minimal encoding (default)
script.number.decode(new Uint8Array([0x7f]));       // 127
script.number.decode(new Uint8Array([0x80, 0x00])); // 128

// Decode with relaxed minimal check
script.number.decode(new Uint8Array([0x00]), 4, false); // 0 (would throw with minimal=true)
```
