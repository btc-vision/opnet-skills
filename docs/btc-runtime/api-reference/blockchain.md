...
| `Blockchain.block.medianTimestamp` | `u64` | Median timestamp (seconds) -- **DO NOT USE for time-dependent logic; miners can manipulate MTP within +/- 2 hours. Use `block.number` instead.** |

```typescript
const blockNum = Blockchain.block.number;
// ⚠️ medianTimestamp exists but should NOT be used for time-dependent logic
...
| `block.timestamp` | `Blockchain.block.number` (use block height, NOT medianTimestamp -- miners can manipulate MTP) |
