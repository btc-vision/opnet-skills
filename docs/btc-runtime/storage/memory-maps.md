...
contract Staking {
    ...
    function calculateReward(address user) public view returns (uint256) {
        uint256 durationBlocks = block.number - stakedBlock[user];
        return stakedAmount[user] * durationBlocks / BLOCKS_PER_YEAR;
    }

    function getStakeInfo(address user) external view returns (uint256, uint256, uint256) {
        return (stakedAmount[user], stakedBlock[user], rewards[user]);
    }
}
```
...
export class Staking extends OP_NET {
    ...
    public stake(calldata: Calldata): BytesWriter {
        ...
        this.stakedBlock.set(sender, u256.fromU64(Blockchain.block.number));
    ...
    private calculateReward(user: Address): u256 {
        ...
        const durationBlocks = SafeMath.sub(currentBlock, stakedAtBlock);
        const staked = this.stakedAmount.get(user);

        // Simplified: staked * durationBlocks / BLOCKS_PER_YEAR
        // ~52,560 blocks per year on Bitcoin (10 min/block)
        const BLOCKS_PER_YEAR = u256.fromU64(52560);
        return SafeMath.div(SafeMath.mul(staked, durationBlocks), BLOCKS_PER_YEAR);
    }
...
public stake(calldata: Calldata): BytesWriter {
    ...
    // Update stake time
    this.stakedTime.set(sender, u256.fromU64(Blockchain.block.number));
