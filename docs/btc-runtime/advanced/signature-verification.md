...
| Signature deadline enforcement | Manual | `Blockchain.block.number` (use block height, NOT medianTimestamp -- miners can manipulate MTP) |
...
```typescript
// Limit signature validity (use block height, NOT medianTimestamp)
if (Blockchain.block.number > deadline) {
    throw new Revert('Signature expired');
}
```
