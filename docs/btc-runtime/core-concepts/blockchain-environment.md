...
// ⚠️ DO NOT USE for time-dependent logic -- miners can manipulate MTP within ±2 hours
...
| `block.timestamp` | `Blockchain.block.number` (use block height, NOT medianTimestamp -- miners can manipulate MTP) | Block height is strictly monotonic and tamper-proof |
...
OPNet exposes **Median Time Past (MTP)** via `Blockchain.block.medianTimestamp`, but **you should NOT use it for time-dependent logic** such as deadlines, staleness checks, or duration calculations. Use `Blockchain.block.number` instead.

```typescript
// Block-based logic (RECOMMENDED)
const currentBlock: u64 = Blockchain.block.number;
const lastBlock = this.lastUpdate.value.toU64();
const BLOCKS_24H: u64 = 144; // ~24 hours on Bitcoin (10 min/block)
if (currentBlock - lastBlock > BLOCKS_24H) {
...
export class MyContract extends OP_NET {
    ...
    public doSomething(calldata: Calldata): BytesWriter {
        ...
        // Block height check (use block.number, NOT medianTimestamp)
        const lastBlock = this.lastUpdate.value.toU64();
...
| `Blockchain.block.medianTimestamp` | `u64` | Median time past -- **DO NOT USE for time-dependent logic** |
